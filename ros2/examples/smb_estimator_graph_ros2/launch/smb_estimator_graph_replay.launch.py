...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("imu_topic_name", default_value="/imu", description="IMU topic name"),
            DeclareLaunchArgument(
                "lidar_odometry_topic_name", default_value="/mapping/scan2map_odometry", description="Lidar odometry topic name"
            ),
            DeclareLaunchArgument(
                "core_graph_param_file", default_value=default_core_graph_param_file, description="Core graph parameter file"
            ),
            DeclareLaunchArgument(
                "smb_graph_param_file", default_value=default_smb_graph_param_file, description="SMB graph parameter file"
            ),
            # Include the main launch file
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource([os.path.join(pkg_dir, "launch", "smb_estimator_graph.launch.py")]),
                launch_arguments={
