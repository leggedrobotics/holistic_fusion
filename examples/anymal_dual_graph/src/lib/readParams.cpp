...
void printKey(const std::string& key, T value) {
  std::cout << YELLOW_START << "AnymalEstimator " << COLOR_END << key << "  set to: " << value << std::endl;
}
template <>
void printKey(const std::string& key, std::vector<double> vector) {
  std::cout << YELLOW_START << "AnymalEstimator " << COLOR_END << key << " set to: ";
...
T tryGetParam(const std::string& key, const ros::NodeHandle& privateNode) {
  ...
  if (privateNode.getParam(key, value)) {
    ...
    throw std::runtime_error("AnymalEstimator - " + key + " not specified.");
  }
}
...
void AnymalEstimator::readParams_(const ros::NodeHandle& privateNode) {
  ...
  // Set frames
  /// IMU
  std::string frame = tryGetParam<std::string>("extrinsics/imuFrame", privateNode);
  staticTransformsPtr_->setImuFrame(frame);
  /// World
  frame = tryGetParam<std::string>("extrinsics/worldFrame", privateNode);
  dynamic_cast<AnymalStaticTransforms*>(staticTransformsPtr_.get())->setWorldFrame(frame);
  /// Map
  frame = tryGetParam<std::string>("extrinsics/mapFrame", privateNode);
  dynamic_cast<AnymalStaticTransforms*>(staticTransformsPtr_.get())->setMapFrame(frame);
  /// Odom
  frame = tryGetParam<std::string>("extrinsics/odomFrame", privateNode);
  dynamic_cast<AnymalStaticTransforms*>(staticTransformsPtr_.get())->setOdomFrame(frame);
  /// base_link
  ...
  /// Gnss frame
  frame = tryGetParam<std::string>("extrinsics/gnssFrame", privateNode);
  ...
  gnssRate_ = tryGetParam<double>("sensor_params/gnssRate", privateNode);
  ...
  // TODO: rename to MapToOdom
  graphConfigPtr_->relocalizationAtStartFlag = tryGetParam<bool>("common_params/relocalizeWorldToMapAtStart", privateNode);
  ...
  if (graphConfigPtr_->usingGnssFlag) {
    ...
    const double initYaw = tryGetParam<double>("gnss/initYaw", privateNode) * M_PI / 180;
  ...
  trajectoryAlignmentHandlerPtr_->setNoMovementTime(tryGetParam<double>("trajectoryAlignment/noMovementTime", privateNode));
}
