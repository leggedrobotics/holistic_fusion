...
bool StaticTransformsTf::findTransformations() {
  ...
  REGULAR_COUT << " Waiting for up to " << kDefaultTfLookupTimeoutSeconds << " seconds until they arrive..." << std::endl;
  ...
  try {
    ...
    listener_.waitForTransform(imuFrame_, baseLinkFrame_, ros::Time(0), ros::Duration(kDefaultTfLookupTimeoutSeconds));
    ...
    REGULAR_COUT << RED_START << " Failed to look up required base transform: " << e.what() << COLOR_END << std::endl;
    return false;
  }
