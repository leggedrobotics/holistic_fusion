...
void GraphMsfRos::readParams(const ros::NodeHandle& privateNode) {
  ...
  std::cout << YELLOW_START<<"Imu rate: "<<graphConfigPtr_->imuRate_<<", create state every: "<<graphConfigPtr_->createStateEveryNthImuMeasurement_<<" , maxsearch Deviation: "<<graphConfigPtr_->maxSearchDeviation_<<std::endl;
  ...
  graphConfigPtr_->logLatencyAndUpdateDurationToMemoryFlag_ =
      tryGetParam<bool>("common_params/logLatencyAndUpdateDurationToMemory", privateNode);
