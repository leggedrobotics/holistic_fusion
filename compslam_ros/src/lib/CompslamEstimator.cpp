...
CompslamEstimator::CompslamEstimator(ros::NodeHandle& node, ros::NodeHandle& privateNode) {
  ...
  // Subscribers
  /// Remapped IMU topic
  subImuCabin_ = node.subscribe<sensor_msgs::Imu>("/imu_topic_cabin", ROS_QUEUE_SIZE, &CompslamEstimator::imuCabinCallback_, this,
                                                  ros::TransportHints().tcpNoDelay());
  std::cout << YELLOW_START << "CompslamEstimator" << COLOR_END << " Initialized IMU cabin subscriber." << std::endl;
  subImuBase_ = node.subscribe<sensor_msgs::Imu>("/imu_topic_base", ROS_QUEUE_SIZE, &CompslamEstimator::imuBaseCallback_, this,
                                                 ros::TransportHints().tcpNoDelay());
  std::cout << YELLOW_START << "CompslamEstimator" << COLOR_END << " Initialized IMU base subscriber." << std::endl;
  ...
  /// Subscribe to m545 measurements
  subMeasurements_ = node.subscribe<m545_msgs::M545Measurements>(
      "/measurement_topic", ROS_QUEUE_SIZE, &CompslamEstimator::measurementsCallback_, this, ros::TransportHints().tcpNoDelay());
  std::cout << YELLOW_START << "CompslamEstimator" << COLOR_END << " Initialized Measurements subscriber." << std::endl;
...
void CompslamEstimator::lidarOdometryCallback_(const nav_msgs::Odometry::ConstPtr& odomLidarPtr) {
  ...
  if (!areYawAndPositionInited_() && (secondsSinceStart_() > 15)) {
...
void CompslamEstimator::publishState_(ros::Time imuTimeK, const Eigen::Matrix4d& T_W_O, const Eigen::Matrix4d& T_O_Ik,
  ...
  tf::poseTFToMsg(tf_T_O_I, poseStamped.pose);
  /// Path
